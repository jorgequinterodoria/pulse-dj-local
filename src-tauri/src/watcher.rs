...
pub fn start_fs_watcher(
    ...
    std::thread::spawn(move || {
        ...
        if let Ok(conn) = Connection::open(&djay_db_path) {
            ...
            // RESTAURADO EL ORDEN EXACTO DE PYTHON: Prioriza 'a.' (AnalyzedData) sobre 'i.' (MediaItemIndex)
            ...
            ") {
                ...
                let _ = stmt.query_map([], |row| {
                    ...
                    // Redondeo idéntico al round(bpm, 1) de Python
        ...
        println!("🎧 [2/2] Iniciando Pulse DJ Bridge (Motor 100% Rust - En Vivo)...");

        loop {
            std::thread::sleep(Duration::from_millis(1000));

            let conn = match Connection::open(&djay_db_path) {
                Ok(c) => c,
                Err(_) => continue,
            };
            let _ = conn.busy_timeout(Duration::from_secs(5));
            ...
            let _ = stmt_db2.query_map([], |row| {
                ...
                if let Ok(Some(d)) = row.get::<usize, Option<Vec<u8>>>(0) {
                    row_text.push_str(&String::from_utf8_lossy(&d));
                }
                if let Ok(Some(m)) = row.get::<usize, Option<Vec<u8>>>(1) {
                    row_text.push_str(&String::from_utf8_lossy(&m));
                }
            ...
            // MISMO ORDEN DE SQL PARA LA BÚSQUEDA EN VIVO
            ...
            for row_text in recent_rows {
                ...
                for track in &library {
                    if row_text.contains(&track.title) && row_text.contains(&track.artist) {
            ...
            if let Some(new_track) = best_match {
                ...
                if *last_id != new_track.id {
                    ...
                    *last_id = new_track.id.clone();
                    ...
                    println!("🎵 Rust Engine detectó: {} - {} [{} | {} BPM]", new_track.title, new_track.artist, new_track.key, new_track.bpm);
